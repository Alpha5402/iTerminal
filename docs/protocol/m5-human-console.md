...
| `GET`    | `/api/sessions/:id/secret-input?generation=`                      | metadata-only `secret.input.get`; no secret bytes                         |
| `POST`   | `/api/sessions/:id/secret-input`                                  | Human-only `secret.input.begin`; transient secret plus metadata           |
| `POST`   | `/api/sessions/:id/secret-input/:sensitiveInputId/finish`         | exact-Human, expected-version `secret.input.finish`                       |
...
The secret-input form uses an HTML password field but does not treat that UI affordance as the security boundary. `POST /secret-input` sends transient bytes to a server-held Human Actor and creates only a metadata-only `SecretInputAction`; handlers must not log the request body or echo validation data. The Runtime enables Executor redaction before the PTY write. While the period is active it rejects ordinary Input and new Execute, permits only the exact opening Human to send explicit Control such as Ctrl+C, and suppresses all raw PTY output before Runtime rings, Events, Artifacts, Screen, WebSocket, and MCP observation. The same Human explicitly completes or cancels the period using its current version; disconnect, timeout, or Execution exit does not disable redaction. The fixed notice and later Virtual Screen are redacted observations, not reconstructions of hidden terminal cells. See ADR-0050 for delivery uncertainty and scope limits.
