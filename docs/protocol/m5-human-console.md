...
- Allowed bind hosts: one configured `127.0.0.0/8` literal, `::1`, or `localhost` only.
- Host hostname must equal that configured name and its effective port must match the connected
  listener. Alternate loopback aliases are rejected.
- State-changing HTTP and WebSocket upgrade require an exact scheme/hostname/port `Origin` with no
  credentials, path, query, or fragment.
- Every HTTP `/api/*` request, including bootstrap and observation GETs, requires
  `X-ITerminal-Request: console`.
- Browser Fetch Metadata, when present, must be `none` or `same-origin`.
...
`requestId` is generated by the Console. A caller-supplied `X-Request-Id` is ignored and never
reflected. Schema, malformed-JSON, body-limit, Host, and Origin failures use fixed messages and do
not return raw parser/Zod issues or input values.
...
Client frames are capped at 16 KiB. The first malformed acknowledgement closes with `1008`; an
oversized frame closes with `1009`. The server does not retain a parse/error-response loop for an
invalid client.
