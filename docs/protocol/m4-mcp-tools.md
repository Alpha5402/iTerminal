...
M9.4 replaces Router-side list-and-pick with an atomic PostgreSQL placement claim. New root Sessions are assigned by monotonic attempt count across unexpired ACTIVE owners; DRAINING owners serve only exact existing routes. M9.15 refines the ordering to the exact `placement_count / capacity_weight` ratio, preserving equal round-robin behavior at the default weight 1 while allowing explicit relative shares. It remains attempt-based scheduling, not active-load measurement or a hard Session cap.
