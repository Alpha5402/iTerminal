...
M9.1–M9.10 已完成 registry、central Router forwarding、Session lease/fencing、公平 placement、durable rate limit、独立进程 chaos、单 owner/单 Router 数据库路径分区隔离、Router degraded cold-start recovery、Router claim/幂等 mutation in-flight crash 与 durable root Session 创建幂等性。旧 generation 的 PTY 永不迁移；后续 M9 工作是 PostgreSQL minority/quorum、持续 rolling drain 与 soak 验证。
...
- [ ] PostgreSQL minority/quorum、hostile-key retention/cardinality、持续 rolling drain、CPU-starved heartbeat 与 long soak；完成前 M9 不升级为 L4。
