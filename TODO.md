...
> 状态：Implementation Baseline v6.15 — M5/M6.6/M7.2 已通过 Browser Human L3；M6.7、M7.1 与 M9.1–M9.15 owner/routing/fencing/weighted-placement/rate-limit/process/partition/Router-recovery/crash/bounded-root-idempotency/drain-settlement/rolling-replacement/CPU-starvation 路径已通过 real PTY/PostgreSQL L2；M4 autonomous-model L3 仍待显式授权
>
> 基线日期：2026-08-30
>
> 当前仓库状态：M0–M4.1 已实现并保存 L2 证据；live Runtime 已接入 PostgreSQL write-ahead journal、bounded ingest loop、versioned dynamic ANSI/VT Virtual Screen，以及 generation-scoped Input Policy/Interaction Guard。M5/M6.6 增加 loopback Human Console 与受控 ResizeAction：真实无头 Chrome Human 和 official MCP SDK Agent 已在同一 PostgreSQL/zsh Session 中共享 cwd/env/Python REPL，并分别驱动同一 PTY 的 SIGWINCH。M6.7 增加不可作为安全事实的 exact-generation `terminal_state`。M7.1 增加 Shell READY control-channel checkpoint、运维 exact allowlist env、version/hash/CAS/staleness、cwd containment、PostgreSQL `session_forks`/child lineage/idempotency 与 `session_checkpoint|session_fork` MCP；真实 bash/zsh 中 READY/RUNNING/BROKEN parent 可重建独立 child，不复制 process/REPL/editor/alias/function/trap，也不隔离共享 workspace 文件。M7.2 在同一 stable owner 的新 daemon 中 hydrate 最多 100 个只读历史 BROKEN projection；Human Console 明示 stale/non-copy 边界并重建新 Session/PTY，真实 Chrome 路径已在恢复 cwd/env 后运行 `git status`。M8.9 已证明 queue-driven owner-local Execute、Input/Control 写后 owner 崩溃不重放、DB/AMQP 恢复与真实三节点 RabbitMQ quorum leader failover。M9.1 新增 PostgreSQL Runtime owner registry：boot-unique instance、单调 registry epoch、heartbeat、drain、DB-time expiry 与 stale identity 拒绝；同 owner 冲突进程在 durable recovery 前 fail closed，失去 registry identity 的 daemon 会 owner-wide 熔断并关闭本地 PTY。M9.2 新增 stable local Unix Router：使用单语句 PostgreSQL route resolution 将 Session/Execution 操作转发到 exact owner，DRAINING 只服务既有 Session，缺失/过期 route fail closed；official MCP 与显式 router-mode Execution Worker 已跨两个真实 Runtime/PTY。M9.3 新增 generation-scoped Session lease/fencing token：root/fork acquisition、exact-set renewal、close/broken release 与所有 live durable mutation 在事务中校验 current owner incarnation + Session fence；Execution 再独立使用 expected version。stale owner/revoked lease 写入被拒绝，旧本地 PTY/process group best-effort 关闭，恢复只能创建新 Session，不能接管旧 generation。M9.4 使用 PostgreSQL 原子 claim 按单调 placement attempt 在 ACTIVE owner 间公平轮转，并将 Actor/Session fixed-window Action 限流放入 admission 同一事务；真实三 Runtime 并发创建为 4/4/4，drain 后剩余两 owner 为 3/3，跨 owner Actor 与单 Session 多 Actor 限流均通过。M9.5 将 Router 与三个 Runtime 放入独立 OS 进程：Router `SIGKILL` 可无状态重启，owner `SIGKILL` 后旧 Session/Execution 仅恢复为 `BROKEN/UNKNOWN`，same-owner epoch 2 只能承接新 Session/PTY，另一 owner `SIGTERM` 后持久化 `STOPPED` 且 placement 继续只落到 ACTIVE owners。M9.6 再将单一 owner 置于 PostgreSQL silent blackhole：仅受害 owner 熔断并在 lease expiry 后移出 placement，健康 owners 持续执行/分配；stale TCP 流 reset/reconnect 后同一 process/instance 保持 epoch 1，旧 Session/Execution 为 `BROKEN/UNKNOWN`，仅新 Session 创建新 PTY。M9.7 在 placement claim commit 后/owner forward 前与幂等 `execution.start` 成功后/Router response 前分别 `SIGKILL` 独立 Router：前者保留 attempt 且无 ghost Session，后者以原幂等键收敛为一个 Action、一个 Execution 和一次真实 Shell 副作用。M9.8 将 caller root-create key、request hash、exact owner incarnation 与最终 Session ID 持久绑定；post-forward Router `SIGKILL` 后同 key 返回原 Session，两个 Router 并发重放只消费一次 placement/创建一个 PTY，变更 payload 被拒绝。M9.9 将单一 Router 置于 PostgreSQL silent blackhole：受害 Router 有界 fail closed 且不 forward/placement，健康 Router 与两个 owners 持续执行/创建；stale TCP reset 后原 Router process 以同 key 创建唯一 Session。M9.10 让生产 Router 在 PostgreSQL 不可达的 cold start 中先绑定 stable socket，以 CONNECTING/UNAVAILABLE gate 拒绝路由并后台重试 migration/health；恢复后原 process 转 READY，degraded create 不产生 intent/placement/PTY。M9.11 以 PostgreSQL singleton policy 为所有 Router/direct durable fallback 统一 root-create retention/capacity；active/live intent 不按年龄删除，新 key 满载时在 placement 前 `BACKPRESSURE`，terminal/stale intent 到期后有界回收，过期 key明确开始新的 Session contract。M9.12 将 owner 先持久化为 `DRAINING`，在同一有界 deadline 内结算 exact-owner pre-drain create intent 并排空已接受 RPC response，随后关闭 Session 并持久化 `STOPPED`；timeout 不重分配 intent。M9.13 在 48 个并发 root-create 中轮流 drain/replace A/B/C 两次，所有 key 唯一绑定、每轮健康 owner 完成真实 zsh、最终三 owner 均为 ACTIVE/epoch 3。M9.14 将 DB-time expiry 加入 heartbeat/drain/stop 原子前置条件；真实 `SIGSTOP` owner 不能短暂续活路由，旧 PTY 被回收并经完整 reconcile 才恢复新 Session，健康 owner 全程执行。M9.15 新增 1–1000 relative capacity weight，PostgreSQL 按 `placement_count / capacity_weight` 原子选择；真实 1:2:3 owners 经 drain/replacement 后累计 6/12/18 并各自执行 zsh。PostgreSQL minority/quorum、remote process reclamation 与 long-duration soak 仍未实现。Autonomous model 授权、更广 TUI/跨浏览器/style parity、daemon restart 后 durable wait、Approval/secret 与完整 MVP/L4 仍未完成。
>
...
- [x] `executions`：execute action、generation、owner、write/start/end state、exit/outcome/unknown reason、expected version 与 Session fencing。
...
- [x] `session_leases`：generation-scoped、exact owner incarnation、全局单调 fencing token、DB-time acquire/renew/expiry/release/version；registry epoch 不替代 Session token。
...
- [x] 所有 Execution 状态更新使用 expected version，并在同一事务额外校验 current owner incarnation + generation-scoped fencing token。
...
M9.1–M9.15 已完成 registry、central Router forwarding、Session lease/fencing、capacity-weighted placement、durable rate limit、独立进程 chaos、单 owner/单 Router 数据库路径分区隔离、Router degraded cold-start recovery、Router claim/幂等 mutation in-flight crash、有界 durable root Session 创建幂等性、pre-drain placement 结算、六轮 rolling owner replacement，以及 CPU-starved owner expiry fencing/recovery。旧 generation 的 PTY 永不迁移；后续 M9 工作是 PostgreSQL minority/quorum、remote reclamation 与 long-duration soak 验证。
...
- [x] Central Router 的 stable Unix RPC、单语句 Session/Execution owner 解析、ACTIVE placement、DRAINING existing-route、全量 list 与 fail-closed 转发（M9.2 L2）。
- [x] generation-scoped Lease、exact-set renewal、DB-time expiry/release 与全局单调 fencing token（M9.3 L2）。
- [x] 所有 live durable mutation 校验 exact owner incarnation/session/generation/fencing；Execution 状态再校验 expected version（M9.3 L2）。
- [x] 非 current owner incarnation 无法提交第二个同 Session generation 的 durable mutation；lease loss 关闭本地 PTY，recovery 只允许新 Session（M9.3 L2）。
- [x] owner lost -> old generation BROKEN；checkpoint rebuild 创建新 Session ID/generation 1 并保留 immutable lineage（same-owner M7.2）。
- [x] stale owner/revoked Session lease 的 DB writes 被事务围栏拒绝；daemon owner-wide 熔断并 best-effort 关闭旧 executor/process group（M9.3 L2；host partition 后远端回收不在本切片证明范围）。
- [x] 原子 root-Session placement claim：3 个真实 ACTIVE Runtime 并发 12 次为 4/4/4，DRAINING owner 被排除，随后剩余两个 owner 6 次为 3/3（M9.4 L2；按单调 attempt 公平，不是容量/活跃负载调度）。
- [x] PostgreSQL-time fixed-window per-Actor/per-Session Action rate limit：跨 owner 共享 Actor、单 Session 多 Actor、幂等 replay、事务 rollback、retry metadata 与窗口重置（M9.4 L2）。
- [x] 独立 OS 进程三 owner chaos：12 Session 4/4/4、Router `SIGKILL` 无状态重启、Runtime `SIGKILL` + same-owner epoch replacement、旧 generation `BROKEN/UNKNOWN` 且不接管 PTY、graceful `SIGTERM` 后 `STOPPED`/placement 排除（M9.5 L2）。
- [x] 单 owner PostgreSQL silent blackhole：受害 owner 熔断/lease expiry，健康 owners 持续执行和 placement；reset/reconnect 后旧 generation `BROKEN/UNKNOWN`、新 Session 可用（M9.6 L2）。
- [x] Router in-flight crash：claim commit 后/forward 前保留 attempt 且无 ghost Session；幂等 Execute owner 成功后/response 前崩溃以原 key 收敛为一个 Action/Execution/真实 Shell 副作用（M9.7 L2）。
- [x] durable root Session 创建幂等性：caller key/hash 与 exact owner intent 原子 claim，owner 创建事务绑定 Session ID；post-forward Router `SIGKILL`、conflicting reuse 与双 Router 并发 replay 收敛为一次 placement、一个 Session/PTY（M9.8 L2）。
- [x] Router-only PostgreSQL silent blackhole：受害 Router 有界返回 route-phase `RUNTIME_UNAVAILABLE` 且不 forward/placement，健康 Router/owners 持续服务；stale stream reset 后原 Router process 恢复（M9.9 L2；不等于 PostgreSQL minority/quorum）。
- [x] Router PostgreSQL cold-start supervision：stable socket 先以 CONNECTING/UNAVAILABLE gate 监听，后台 migration/health/backoff；恢复后同 process READY，degraded create 无 intent/placement/PTY（M9.10 L2）。
- [x] root-create hostile-key retention/cardinality：PostgreSQL authoritative policy、全局 capacity、terminal/stale eligibility、active/live pinning、direct fallback 同界与 placement 前 `BACKPRESSURE`（M9.11 L2）。
- [x] bounded Runtime drain settlement：先持久化 `DRAINING`，结算 exact-owner pre-drain root-create intent，再排空已接受 RPC response，关闭 Session 并持久化 `STOPPED`；timeout 不重分配 intent（M9.12 L2）。
- [x] repeated rolling owner drain：A/B/C 两轮共六次 drain/replacement，48 个并发 root-create 全部唯一绑定，每轮健康 owner 真实 zsh 进展，最终三 owner ACTIVE/epoch 3（M9.13 L2；不等于 long soak 或 live PTY migration）。
- [x] CPU-starved owner heartbeat：普通 heartbeat/drain/stop 不得延长 DB-time 已过期 identity；真实 `SIGSTOP/SIGCONT` 后旧 PTY/BROKEN/released 与 same-process full recovery、新 Session-only PTY（M9.14 L2）。
...
- [ ] PostgreSQL minority/quorum、remote host reclamation、高基数/长时间 rolling drain 与 long soak；完成前 M9 不升级为 L4。
...
15. [x] `feat(router): route exact sessions to live owners`：stable Router Unix RPC 读取 durable Session/Execution owner 与 live registry route；official MCP 与 router-mode Worker 跨两个 owner，缺失/过期/STOPPED route fail closed，不创建替代 PTY（M9.2 L2）。
16. [x] `feat(runtime): fence generation-scoped session writes`：独立 Session lease/fencing token；所有 live durable mutation 同事务校验 owner/instance/epoch/session/generation/token，Execution 另校验 expected version；stale owner/revoked lease 触发本地 PTY 熔断与新 Session rebuild（M9.3 L2）。
17. [x] `feat(router): balance placement and rate-limit actions`：PostgreSQL 原子 placement claim、ACTIVE-only drain 语义、durable Actor/Session fixed-window counters、`RATE_LIMITED` retry contract 与三 owner real PTY/PostgreSQL L2 证据。
18. [x] `test(runtime): exercise independent owner process chaos`：独立 Router/三 Runtime OS 进程、Router/owner `SIGKILL`、same-owner replacement fencing、graceful drain/stop 与 replacement-only 新 PTY 的 L2 组合证据。
19. [x] `fix(runtime): survive asymmetric owner database partition`：per-owner TCP blackhole、checked-out PostgreSQL Client error 防崩、健康 owner 连续服务与 reset/reconnect 后新 PTY（M9.6 L2）。
20. [x] `test(router): preserve in-flight crash truth`：claim-before-forward 与 idempotent mutation-before-response 两个独立 Router `SIGKILL` 边界，保持 attempt/Action/Execution/Shell effect 可核验（M9.7 L2）。
21. [x] `feat(router): make root session creation idempotent`：caller key、durable placement intent、owner-transaction Session binding、post-forward `SIGKILL` settlement 与双 Router concurrent replay（M9.8 L2）。
22. [x] `fix(router): fail database partitions closed`：Router route-database error contract、双 Router/双 owner silent-blackhole isolation、healthy-path progress 与 in-process recovery（M9.9 L2）。
23. [x] `fix(router): recover from database cold starts`：degraded RPC gate、migration/health supervisor、bounded reconnect、healthy Router progress 与 same-process READY recovery（M9.10 L2）。
24. [x] `feat(persistence): bound root session idempotency`：database-authoritative retention/capacity、active/in-flight pinning、terminal/stale cleanup、direct fallback 与 expired-key 新 Session contract（M9.11 L2）。
25. [x] `fix(runtime): settle pre-drain placements`：DRAINING-before-close、exact-owner pending-intent observation、bounded RPC response drain，以及双 Runtime/真实 zsh 独立进程结算证据（M9.12 L2）。
26. [x] `test(runtime): exercise repeated rolling drains`：三 stable owner 两轮 drain/replacement、48 个并发 durable create、每轮 healthy-owner Shell progress 与 epoch 3 最终审计（M9.13 L2）。
27. [x] `fix(runtime): fence expired owner heartbeats`：heartbeat/drain/stop DB-time expiry predicate、真实 `SIGSTOP/SIGCONT`、healthy-owner progress、旧 Shell 回收与 same-process reconcile/new-PTY-only recovery（M9.14 L2）。
