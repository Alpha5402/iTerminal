...
M9.1–M9.7 已完成 registry、central Router forwarding、Session lease/fencing、公平 placement、durable rate limit、独立进程 chaos、单 owner 数据库分区隔离与 Router claim/幂等 mutation in-flight crash。旧 generation 的 PTY 永不迁移；后续 M9 工作是 root Session 创建幂等性、minority/Router partition、持续 rolling drain 与 soak 验证。
...
- [ ] post-forward root-Session response loss/idempotency、minority/Router partition、持续 rolling drain、CPU-starved heartbeat 与 long soak；完成前 M9 不升级为 L4。
