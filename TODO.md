...
> 状态：Implementation Baseline v6.34 — M10.14 已闭合 native Shell line-editor dispatch、Shell prompt 原位 READY command line 与 RUNNING password/passphrase 原位 Human-only secret input；Console 已改为顶部多 Session 标签页、`+` 默认创建 `/` 下的 zsh，并在右侧直接复制完整 MCP JSON 而不暴露私有配置路径；M10.13 已闭合一条命令 durable local stack 与 Ctrl+C-safe detached Guardian shutdown；M10.12 已闭合 normalized-fact retention/capacity signal；M9.18 已闭合本机 failure/pressure L4 gate；repository release L4 仍待完成
>
> 基线日期：2026-08-31
>
> 当前仓库状态：M0–M9 与 M10.1–M10.12 的既有实现和证据保持不变。M10.13 新增 `pnpm local` supervisor、loopback PostgreSQL 17 Compose/named volume、既有 durable Runtime + Console composition、24 小时 least-privilege MCP/Console grants、`0600` generic MCP config、Console 右侧完整 JSON handoff、live Console rebuild asset routing、external PostgreSQL mode、`local:stop` recovery，以及 detached Guardian process group。M10.14 让 bash readline/zsh ZLE 直接呈现并提交用户命令，内部 token 留在私有 dispatch/control boundary；Console READY command editor 与 RUNNING secret editor 均覆盖在 Runtime screen cursor 坐标，后者仍经 Human-only SecretInputAction 提交且不回显，secret bytes 不进入 Action/Event/Screen/recording；底栏不再承载普通命令或密码输入，也不再显示 `__it_execute` wrapper。Console 主工作区现以顶部标签页切换多个 Session，最右侧 `+` 固定创建 workspace root `/` 的 zsh Session，Session 列表自动刷新。真实 official MCP + zsh + Console + PostgreSQL 场景证明 Ctrl+C 后 Session 为 `CLOSED`。Actor/Session/Generation lineage retention、Artifact/recording export/legal hold、filesystem/tablespace hard quota、always-on alert/scheduler、clean-machine/platform/client matrix 与 release/dogfood 仍未完成。真正整机/VM fencing、跨主机/跨平台 soak、autonomous model 授权、daemon restart 后 durable wait 与 repository release L4 仍未证明。
>
...
- [x] `actors`：immutable identity/type/canonical capability；冲突 ID fail closed（M10.1 L2）。
- [ ] `session_actors` 与 display metadata / membership lifecycle。
...
- [x] `artifacts`：大输出 metadata/hash/size/retention 已完成；录制/导出仍待 M10。
...
- xterm 显示 Shell output/prompt；READY 时隐藏不可输入的 xterm 光标，并在 Runtime screen cursor 的行列位置覆盖单行 command composer，让输入直接衔接真实 prompt。
...
- [x] Shell prompt 原位 contenteditable command composer、Input、Control、stream wait、PTY_BUSY allowed-next-action 建议。
...
- [x] Shell prompt 原位 READY command line 与 RUNNING interactive focus 分离；READY 隐藏/禁用 xterm stdin，raw input 在 transport 层拒绝。
...
- [x] M10.4 Human-only secret input 与敏感期交互：RUNNING password/passphrase 提示在 Virtual Screen cursor 原位编辑且不回显，未知提示可显式开启；metadata-only Action、exact Human finish、普通 Input 阻断、Human Control 保留、disconnect/Execution exit fail-closed。
...
- [x] M10.7 bounded cursor-safe Event retention：database-authoritative age/count/batch policy；每次只处理一个 generation 的一个 numeric contiguous prefix、保留最新 anchor；durable deletion watermark 让 stale nonzero cursor 显式 `RESYNC_REQUIRED`，公开 operator command 只输出聚合 metadata（L2；active generation 不 pin，不等于 normalized facts retention/whole-database disk bound）。
...
- [x] M10.14 native prompt dispatch：ADR-0060；READY command 与 RUNNING secret editor 均覆盖 Runtime screen cursor，Enter 分别提交单次 ExecuteAction 与 Human-only SecretInputAction；zsh ZLE 与 macOS bash 3.2 readline/history recall 将用户原命令装入真实 Shell 行编辑器，私有 command/token 文件 + control FIFO + PTY barrier 保留完成顺序，屏幕不再泄漏 `__it_execute`/execution id（bash/zsh persistent state、多行、syntax error L2；真实 Chrome zsh + official MCP/secret redaction L3；不等于 user rc/plugin compatibility）。
...
40. [x] `feat(local): bootstrap durable stack in one command`：managed loopback PostgreSQL、existing durable Runtime/Console、private generated MCP config、external DB mode、idempotent signal shutdown 与 detached Guardian process group（M10.13 L2；不等于 clean-machine/client matrix/production topology）。
