...
M9.1–M9.11 已完成 registry、central Router forwarding、Session lease/fencing、公平 placement、durable rate limit、独立进程 chaos、单 owner/单 Router 数据库路径分区隔离、Router degraded cold-start recovery、Router claim/幂等 mutation in-flight crash，以及有界 durable root Session 创建幂等性。旧 generation 的 PTY 永不迁移；后续 M9 工作是 PostgreSQL minority/quorum、持续 rolling drain 与 soak 验证。
