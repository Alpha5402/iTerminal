...
M9.1–M9.6 已完成 registry、central Router forwarding、Session lease/fencing、公平 placement、durable rate limit、独立进程 chaos 与单 owner 数据库分区隔离。旧 generation 的 PTY 永不迁移；后续 M9 工作是 minority/Router partition、in-flight crash、持续 rolling drain 与 soak 验证。
...
- [ ] minority/Router partition、in-flight mutation/claim crash、持续 rolling drain、CPU-starved heartbeat 与 long soak；完成前 M9 不升级为 L4。
