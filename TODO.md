...
- [x] M10.14 native prompt dispatch：ADR-0060；READY editor 覆盖 Runtime screen cursor，Enter 仍提交单次 ExecuteAction；zsh ZLE 与 macOS bash 3.2 readline/history recall 将用户原命令装入真实 Shell 行编辑器，私有 command/token 文件 + control FIFO + PTY barrier 保留完成顺序，屏幕不再泄漏 `__it_execute`/execution id（bash/zsh persistent state、多行、syntax error L2；真实 Chrome zsh + official MCP L3；不等于 user rc/plugin compatibility）。
