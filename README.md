...
The Console defaults to `http://127.0.0.1:4173` and refuses non-loopback binding; build its static assets before starting it. The plain daemon command above starts explicit in-memory development mode. To run the durable shared path, give the daemon `ITERM_DATABASE_URL` and point Console/MCP at one daemon socket or the Router socket. To run the M8/M9 queue path, also start RabbitMQ, use `ITERM_EXECUTION_DISPATCH=external`, and launch the Outbox relay plus an owner-bound or explicit router-mode Execution Worker. `ITERM_RABBITMQ_URLS` accepts an ordered comma-separated broker list and takes precedence over the single-endpoint fallback. PostgreSQL/RabbitMQ timeout and reconnect variables are documented in the roadmap. Stable owner IDs derive from daemon sockets unless explicitly configured; the Router does not register as an owner. PostgreSQL durability does not sandbox Shell commands or resurrect a lost PTY.
