...
M9.1 adds a PostgreSQL Runtime owner registry with stable logical owner IDs, boot-unique instance IDs, monotonic registry epochs, database-time heartbeat leases, drain/stop lifecycle, and stale-identity rejection. Registration happens before durable recovery, so a conflicting same-owner daemon cannot break the live owner's Sessions; loss of the registered identity conservatively destroys local PTY state. The selected central Unix Router, cross-owner forwarding, generation-scoped Session leases/fencing, stale durable-write rejection, and 3+ Worker chaos gate remain unimplemented.
