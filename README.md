...
**M0–M4.1, M6.5 interaction policy/short Human Guards, and the M8.9 local admission/crash/recovery path pass at L2: shared Shell, durable Action Runtime, real stdio MCP, bounded ANSI/VT screen reads/regions/diffs/search/waits/styles, guarded multi-Actor input, reliable queue-driven PTY dispatch, non-replayable uncertain interactions, silent-network recovery, and real three-node RabbitMQ quorum leader failover.**

Real local bash and zsh PTY scenarios prove command boundaries, shared state, fail-fast Busy, structured Input/Control, stale-target rejection, marker-spoof isolation, large output, and Ctrl+C recovery. With `ITERM_DATABASE_URL`, the live daemon commits Execute/Input/Control admission before PTY delivery, sends attributed output through a bounded per-Session ingest loop, serves durable cursors, and marks a `SIGKILL`-lost owner generation `BROKEN/UNKNOWN` on restart. M6.1 projects the same PTY stream through a pinned headless ANSI/VT emulator and exposes a fixed 120×40 normal/alternate viewport, cursor, and version through Runtime RPC and MCP `screen_get`. M6.2 adds current-viewport literal `screen_search` plus reactive `screen_wait`; M6.3 adds cell-safe `screen_region` and a 64-revision `screen_diff` ring; M6.4 maps material cells into an xterm-independent sparse `screen_cells` DTO with explicit palette/RGB colors and standard SGR attributes. M6.5 adds generation-scoped `common | human_guarded | human_only | agent_only`, version-CAS policy/Guard mutations, bounded TTL/renewal, Human-only emergency Guard bypass, PostgreSQL revalidation, and read-only MCP `interaction_get`. Stable remains explicitly different from prompt readiness. An official MCP SDK Client drives the same live zsh across stdio bridge restarts; OpenCode and Claude Code handshakes also pass. In-memory development mode remains available, but no model-driven Agent has been authorized and the Human Console, resize, style diff, hyperlink/image metadata, and durable wait/subscriptions are not complete.
